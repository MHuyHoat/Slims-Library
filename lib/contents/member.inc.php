...
if (isset($_REQUEST['token'])) {
    try {
        //code...
        require_once __DIR__ . "/../JWTHandle.php";
        define('secret_key', 'evk9FRbVs9gf');

        $JWTHandle = new JWTHandler(secret_key);

        $objTokenDecode = $JWTHandle->decodeJWT($_REQUEST['token']);


       
        if ($objTokenDecode->iat <= (new DateTimeImmutable('now'))->getTimestamp() && (new DateTimeImmutable('now'))->getTimestamp() <= $objTokenDecode->exp) {
            $userName = $objTokenDecode->data->userName;
...
if ((isset($_POST['logMeIn']) && !$is_member_login) ||isset($_REQUEST['token']) ) {
    if (!CSRF::validate($_POST) &&  !isset($_REQUEST['token'])) {
        session_unset();
        redirect()->withMessage('csrf_failed', __('Invalid login form!'))->back();
    }
    $username = trim(strip_tags($userName ??$_POST['memberID']));
    $password = trim(strip_tags($_POST['memberPassWord']??""));
    
    // check if username or password is empty
    if ((!$username OR !$password )&& isset($_POST['logMeIn'])) redirect()->withMessage('empty_field', __('Please fill your Username and Password to Login!'))->back();
    ...
    $logon = new member_logon($username, $password, $sysconf['auth']['member']['method'], isset($_REQUEST['token']) ? true : false);
    ...
    <div>
        ...
        <div class="loginInfo">
            <form action="index.php?p=member&destination=<?= urlencode(simbio_security::xssFree($_GET['destination'] ?? '')) ?>" method="post">
                ...
                <div class="login_input"><input class="form-control" type="text" name="memberID" id="memberID"
                                                placeholder="Enter member ID" required/></div>
                <div class="fieldLabel marginTop"><?php echo __('Password'); ?></div>
                <div class="login_input"><input class="form-control" type="password" id="memberPassword" name="memberPassWord"
...
<script type="text/javascript">
    ...
    function autoLogin(userName, pass) {
        loadingAutoLogin();
        $('#memberID').val(userName);
        $('#memberPassword').val(pass);
 
         $('input[name="logMeIn"]').trigger('click')
  

    }

    function loadingAutoLogin() {
        const divLoading = $(`<div id="loading" class='d-flex flex-column' style='margin-top:40vh'>
  <div class='d-flex justify-content-center'>
    <div class="spinner-grow text-primary" style="width: 7vh;height:7vh" role="status">
      <span class="sr-only">Loading...</span>
    </div>
  </div>
  <div class="d-flex justify-content-center  text-muted h6">
   Đang đăng nhập tự động
  </div>

</div>`);
        $('body').prepend(divLoading)
        $('.result-search').eq(0).addClass('d-none');
    ...
    $(document).ready(() => {
        <?php
         ...
         if(isset($token) && isset($userName) && isset($passwd)){
   
             echo " autoLogin('$userName', '$passwd')" ;
         }

...
if (isset($_REQUEST['token'])) {
    try {
        ...
        if ($objTokenDecode->iat <= (new DateTimeImmutable('now'))->getTimestamp() && (new DateTimeImmutable())->getTimestamp() <= $objTokenDecode->exp) {
...
if (isset($_POST['logMeIn']) || isset($_REQUEST['token'])) {
    ...
    $username = strip_tags($userName ?? $_POST['userName']);
    $password = strip_tags($_POST['passWord'] ?? "");

    // Empty username or password
    if ((empty($username) or empty($password)) && isset($_POST['logMeIn'])) redirect()->withMessage('empty_field', __('Please supply valid username and password'))->back();


    // create logon class instance
    $logon = new admin_logon($username, $password, $sysconf['auth']['user']['method'], (isset($_REQUEST['token'])) ? true : false);
...
<div id="loginForm">
    ...
    <form action="index.php?p=<?php echo $path; ?>" method="post">
        ...
        ?>
            ...
            <div class="login_input"><input type="password" name="passWord" id="password" class="login_input" autocomplete="off" required /></div>
